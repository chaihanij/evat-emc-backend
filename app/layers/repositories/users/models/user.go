...
type User struct {
	...
	Image         types.File         `bson:"image"`
	Documents     types.Files        `bson:"documents"`
...
func (u *User) ParseToModel(e *entities.UserCreate) *User {
	...
	if e.Academy != nil {
		user.Academy = *e.Academy
	}
	if e.Branch != nil {
		user.Branch = *e.Branch
	}
	if e.Tel != nil {
		user.Tel = *e.Tel
	}
...
func ToUpdateUserFields(input *entities.UserPartialUpdate) *bson.D {
	...
	if input.LastName != nil {
		updateFields = append(updateFields, bson.E{Key: "last_name", Value: input.FirstName})
	}
	...
	if input.Branch != nil {
		updateFields = append(updateFields, bson.E{Key: "branch", Value: input.Branch})
	}
	if input.Tel != nil {
		updateFields = append(updateFields, bson.E{Key: "tel", Value: input.Tel})
	}
	if input.UserType != nil {
		updateFields = append(updateFields, bson.E{Key: "tel", Value: input.UserType})
	}
	if input.Role != nil {
		updateFields = append(updateFields, bson.E{Key: "role", Value: input.Role})
	}
	if input.Image != nil {
		updateFields = append(updateFields, bson.E{Key: "image", Value: input.Image})
	}
	if input.Documents != nil {
		updateFields = append(updateFields, bson.E{Key: "documents", Value: input.Documents})
	}
