...
func (r repo) UpdateUser(ctx context.Context, userFilter, input *entities.User) (*entities.User, error) {
	ctx, cancel := context.WithTimeout(ctx, env.MongoDBRequestTimeout)
	defer cancel()
	opts := options.FindOneAndUpdate().SetReturnDocument(options.After)
	filter := models.NewUserFilter(input)
	statement := models.UpdateUser(input)
	...
	log.WithField("value", user).Debugln("DB UpdateUser")
